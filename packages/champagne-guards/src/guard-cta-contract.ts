...
import { arbitrateCtaSurfaces } from "../../champagne-sections/src/ctaSurfaceArbitrator";
import { getSectionStack } from "../../champagne-sections/src/SectionRegistry";
import { normalizeSectionsForMidCTA } from "../../champagne-sections/src/ctaPlacement";
import { resolveTreatmentMidCTAPlan } from "../../champagne-sections/src/treatmentMidCtaPlan";
import { resolveTreatmentClosingCTAPlan } from "../../champagne-sections/src/treatmentClosingCtaPlan";
import { getTreatmentPages, type ChampagneTreatmentPage } from "../../champagne-manifests/src/helpers";
import type { CTAContractAudit } from "../../champagne-sections/src/ctaRelation";
import type { SectionRegistryEntry } from "../../champagne-sections/src/SectionRegistry";
...
interface PageCTAResult {
  path: string;
  slug: string;
  midIndex: number;
  closingIndices: number[];
  closingButtons: { label: string; href: string }[];
  midButtons: { label: string; href: string }[];
  midAudit?: CTAContractAudit;
  closingAudit?: CTAContractAudit;
}

const pages: ChampagneTreatmentPage[] = getTreatmentPages();
...
const selfLinkIssues: string[] = [];
const semanticIssues: string[] = [];
const invalidTargets: string[] = [];
const missingClosing: string[] = [];
const duplicateClosing: string[] = [];
const surfaceMap: PageCTAResult[] = [];
...
pages.forEach((page) => {
  ...
  const midEntries = ordered
    ...
    .filter((entry): entry is { section: SectionRegistryEntry; index: number } => Boolean(entry));

  const midPlans = midEntries.map((entry) => ({
    index: entry.index,
    plan: resolveTreatmentMidCTAPlan(entry.section, page.path),
  }));
  ...
  audits.forEach((audit) => {
    ...
    audit.invalidTargets.forEach((issue) =>
      invalidTargets.push(`${page.path}: ${issue.href ?? ""} — ${issue.reason ?? "invalid"}`),
    );
    ...
    audit.alternativeMismatches.forEach((issue) =>
      semanticIssues.push(`${page.path}: ${issue.href ?? ""} has alternatives label without alternative relationship (${issue.relationship})`),
    );
  });
...
if (errors.length > 0) {
  console.error("CTA contract guard failed:\n" + errors.map((err) => `- ${err}`).join("\n"));
  process.exit(1);
}
