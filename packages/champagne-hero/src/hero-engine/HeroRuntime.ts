...
export function getHeroRuntime(options: HeroRuntimeOptions): HeroRuntimeResult {
  ...
  for (const entry of surfaces.surfaceStack) {
    ...
    // Perform a role lookup only when the token is a string. The lookup result
    // is intentionally not stored because it isn't used elsewhere. This avoids
    // unused-variable and index-type errors while preserving the runtime
    // evaluation (including the fallback to "background").
    if (typeof token === "string") {
      SURFACE_ROLE_MAP[token] ?? "background";
    }
