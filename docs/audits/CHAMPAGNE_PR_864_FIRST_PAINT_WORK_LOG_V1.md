...
| `a2cee0acfcc9635385a977f86971db034b1dbb9a` | protected canvas owners can silently gain `!important` priority | P2 | current and valid | Adding `!important` to the default `--surface-canvas` owner preserved the recorded value while causing it to outrank dawn/dusk/night owners. Protected canvas owners now fail with `CANVAS_OWNER_IMPORTANT`; canonical dependency owners likewise cannot use priority and fail with `CANVAS_DEPENDENCY_IMPORTANT`. | `d7a5dda6d2243c774e3d8fac43be839cdd29d2e6` | review at `38b0e1db5b396410c43c7b4efa2a3dc2a8e5275a` confirmed priority remediation but found browser-proof and emission-path gaps |
| `38b0e1db5b396410c43c7b4efa2a3dc2a8e5275a` | first-paint browser proof does not isolate external CSS or compare early/final colour | P2 | current and valid | The former pseudo-filmstrip could sample after warm-cache CSS application and asserted only non-transparency. A new eight-case production-browser matrix blocks service workers, holds every external stylesheet, captures synchronously at body attachment with `readyState: loading`, confirms a real Next CSS request is held, releases and confirms every stylesheet finished, waits for complete load, and compares exact sRGB bytes across phases. A disposable stale snapshot failed at `[35,35,37,255]` early versus `[34,34,36,255]` loaded. | `bece39c70b72d37aa0dfdc567c97ea57c3b61db7` | review at `10de7bd2f15987d3004b8a280a48c9909faa613a` confirmed the browser-proof remediation but found the separate readable-foreground gap |
| `38b0e1db5b396410c43c7b4efa2a3dc2a8e5275a` | emitted critical CSS and layout injection are not validated | P2 | current and valid | TypeScript AST inspection now requires the actual `champagneCriticalPaintCss` export template and exactly one layout import/injection; PostCSS validates the closed two-rule emitted CSS shape. The browser proof independently requires one marked v1 direct-head style, one exact CSS copy, and a non-empty emitted text before and after load. Removing the layout style fails the static guard. | `bece39c70b72d37aa0dfdc567c97ea57c3b61db7` | review at `10de7bd2f15987d3004b8a280a48c9909faa613a` confirmed the emission-path remediation but found the separate readable-foreground gap |
| `10de7bd2f15987d3004b8a280a48c9909faa613a` | dark critical canvas has no readable foreground while external CSS is held or unavailable | P2 | current and valid | The critical seed painted the dark canvas while leaving body text at the browser default. The correction derives the existing canonical `--text-ink-high` expression, emits it with a body/root foreground, validates the exact export and canonical binding, and captures body text at body attachment. All eight withheld-stylesheet cases prove the foreground is applied and has at least 4.5:1 contrast against the critical canvas. No canvas material or final Persian Midnight value changed. | `79db177594660c061e162e2a20188ea16235bc2b` | review at `97a55e464f31f8f51110bb30eef484a8a6cc9e80` confirmed the fallback-foreground remediation but found the separate loaded-foreground-dependency gap |
| `97a55e464f31f8f51110bb30eef484a8a6cc9e80` | loaded overrides of foreground dependencies are absent from the closed ownership ledger | P2 | current and valid | Foreground resolution used a throwaway dependency set, so a later loaded `--text-ink-high` declaration could evade ownership checks. The correction retains and unions the full foreground dependency chain with the canvas dependency ledger. A detached-worktree mutation that appended a later `--text-ink-high` owner failed with `CANVAS_DEPENDENCY_OWNER_UNAPPROVED`; the disposable worktree was then removed. The browser proof also requires loaded contrast and exact early/final foreground and body-text sRGB equality. | `9ff46098815ccbb43cb316f87b479f5607966d90` | review at `c0c7da478029510fc34dda8599b610d4a0ce9742` reported no major issue; no new inline thread |
...
| --- | --- | --- | --- | --- | --- |
| 0 | `c0c7da478029510fc34dda8599b610d4a0ce9742` | Baseline, initial HTML and disposable weakness reproduction; complete loaded-cascade ownership and parser hardening; actual export/layout emission validation; deterministic held-stylesheet browser proof across homepage/treatment, mobile/desktop, and reduced/normal motion; readable canonical foreground with closed loaded-dependency ownership and exact early/final equality. | all earlier static mutations fail; removed layout style fails the AST guard; stale critical snapshot fails exact early/final canvas equality; later foreground override fails with `CANVAS_DEPENDENCY_OWNER_UNAPPROVED`; strict Stage B-equivalent matrix `8 passed`; complete exact-head CI including Stage B and `verify` passed; both Vercel previews passed; disposable mutation worktree and browser artifacts removed | clean exact-head result: `Codex Review: Didn't find any major issues. Bravo.` | no unresolved current Codex finding; Phase 0 gate closed before Phase 1 began |
| 1 | architecture commit `59e9b3e2119c295c659f7e36f69f215ec5daf3b5`; Phase 1 log commit pending | Single-source expression-graph architecture, generated CSS/pure TypeScript boundaries, package subpath, deterministic commands, validation ownership, failure semantics and future edit workflow recorded in `CHAMPAGNE_PR_864_CANVAS_MATERIAL_GENERATION_ARCHITECTURE_V1.md`. Three independent read-only specialists audited the expression graph, adversarial failure model and package/import boundary. | `git diff --check`, `guard:hero`, `guard:canon` and full `verify` pass | pending exact-head review | no implementation or colour change in this phase; the transitional guarded path remains active until atomic Phase 3 replacement |
