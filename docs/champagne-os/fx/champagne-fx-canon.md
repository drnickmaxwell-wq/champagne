...
Codex will wire them to real shaders, WebGPU, Lottie, CSS, etc.
...
4. **PRM (prefers-reduced-motion) always honoured**
...
Use sparingly, only on 1–2 primary CTAs per view.
...
## 3. Motion Behaviours (Naming Canon)

Motion behaviours are defined in `champagne-motion-profiles.json`.

1. `motion.softBreath` – very slow, “breathing” movement for water, mist, waves.  
2. `motion.deepBreath` – slightly stronger version for major hero transitions.  
3. `motion.inkfallDrift` – slow vertical drift for ink textures.  
4. `motion.confidenceShimmer` – short shimmer for CTAs/focus states.  
5. `motion.pulseCalm` – soft scaling “heartbeat” for particles/micro glows.  
6. `motion.parallaxGentle` – ultra-low parallax for 3D-ish layers and glass sheens.  
7. `motion.cardLiftSoft` – hover lift for cards (lift ≤ 8px, scale ≤ 1.03).  
8. `motion.prmFadeOnly` – PRM-friendly alpha transitions only.  
9. `motion.staticStill` – no motion.
