...
export function HeroV3DiagnosticLab() {
  ...
  const enhanced = mode === "h3-3-engine";
  ...
  return (
    <main className={styles.page} data-production-binding="false">
      ...
      <section className={styles.stageShell} style={frameStyle} aria-label="Sacred V2 engine comparison">
        <p className={styles.viewportLabel}>{MODES.find((entry) => entry.id === mode)?.label} · {viewport}</p>
        <div className={styles.stage} data-h3-engine-mode={mode} data-h3-viewport={viewport} ref={surfaceRef}>
          <HeroV3MotionScoreController rootRef={surfaceRef} enabled={enhanced} forceFallback={forceFallback} />
