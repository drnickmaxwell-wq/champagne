...
export function ReconstructionLibrary({ initialReviewDataset, reviewPersistence }: { initialReviewDataset: ReviewDataset; reviewPersistence: ReviewPersistence }) {
  ...
  if (!FOUNDER_REVIEW_CANCELLED || reviewDataset.reviews.length !== 0 || reviewPersistence.productionBinding) {
    throw new Error("A2F requires cancelled Founder review, a 0/8 reconstruction dataset, and productionBinding=false");
  }

  return (
    <section className={styles.library} data-testid="a2-component-library" id="a2-review">
      <header className={styles.libraryHeader}><span>A2F · DIRECTOR FIDELITY CALIBRATION</span><h1>Reconstruct the source before adapting the grammar.</h1><p>Exactly three LOVE references are calibrated in locked source/live frames. The A2R Founder review machinery is preserved but cancelled; no reconstruction disposition will be written in this tranche.</p><strong>MAIN_DIRECTORATE_FIDELITY_REVIEW_REQUIRED · A2R remains {progress.complete} / {progress.total}</strong></header>
      ...
      <div className={styles.libraryLayout}>
        <aside className={styles.componentRail} aria-label="A2F calibration component index">
          {componentIndex.components.map((component) => { const inScope = calibrationIds.includes(component.componentId); return <button key={component.componentId} type="button" disabled={!inScope} aria-current={component.componentId === selectedId ? "true" : undefined} onClick={() => setSelectedId(component.componentId as A2ComponentId)}><span>{inScope ? "A2F CALIBRATION" : "FROZEN · NOT IN A2F"} · SOURCE {component.founderSignal}</span><strong>{component.componentId}</strong><small>{component.semanticRole}</small></button>; })}
        </aside>
        <main className={styles.inspector}>
          <header className={styles.componentHeader}><div><span>A2F_SOURCE_FIDELITY_CALIBRATION</span><h2>{selected.exportName}</h2><p>{selected.semanticRole} · source <code>{selected.sourceCvaId}</code></p></div><div className={styles.authorities}><strong>SOURCE PREFERENCE = {selected.founderSignal}</strong><strong>DIRECTOR FIDELITY = PENDING</strong><small>READY_FOR_FOUNDER_RECONSTRUCTION_REVIEW=false</small></div></header>
          {calibration ? <section className={styles.fidelityWorkbench} aria-label="Director source fidelity workbench">
            ...
            <div className={styles.lockedCanvas} data-mode={compareMode} data-native={viewport === calibration.nativeWidth} style={{ "--source-opacity": overlayOpacity / 100, "--frame-ratio": `${calibration.nativeWidth} / ${calibration.nativeHeight}`, "--exact-width": `${viewport}px`, "--locked-zoom": zoom / 100, "--pan-x": `${pan.x}px`, "--pan-y": `${pan.y}px` } as CSSProperties}>
              ...
              <figure className={styles.liveFrame} data-visible={compareMode !== "BLINK" || !blinkSource}><figcaption>LIVE CODE-NATIVE · no PNG body</figcaption><div><div className={styles.previewFrame} data-testid="a2-component-render" style={{ width: `${viewport}px` }}>{examples[selectedId]}</div></div></figure>
            </div>
            ...
            <div className={styles.stageGate}><strong>DIRECTOR_FIDELITY_PASS_FOR_FOUNDER_REVIEW = NOT GRANTED</strong><span>Stage 1 evidence only. Founder review and promotion remain unavailable.</span></div>
          </section> : <section className={styles.outOfScope}><strong>FROZEN A2 CANDIDATE</strong><p>This component is not one of the three authorised A2F calibration references and has not been repaired.</p></section>}
      ...
      <section className={styles.checkpoint}><div><span>FROZEN · DATASET REVISION {reviewDataset.datasetRevision}</span><h2>Separate A2R checkpoint preserved</h2><p>{reviewDataset.reviews.length} Founder reconstruction decisions. Import, export and persistence code remain in the accepted A2R implementation, but writes are disabled throughout A2F calibration.</p></div><strong>0 / 8 · SOURCE PREFERENCE CORPUS UNCHANGED</strong></section>
    </section>
  );
}
