...
import type { DesignProposal, FounderDesignStudioState, GenerationDomain } from "../data/generative-design-contract";
import {
  CONVERGENCE_LANES,
...
import type { ArtDirectionSelections, ArtDirectionVariant, AtelierPreviewState } from "./AtelierPreviewDocument";

// Preview truth disclosures remain rendered by AtelierPreviewDocument:
// fictional architecture · not St Mary’s House; Interactive 3D remains off.
...
type ArtDirectionSectionId = "home.founder-authority" | "home.team-continuity" | "home.technology-purpose" | "home.heritage-story";

const ATELIER_STORAGE_KEY = "champagne.atelier.r4.3.founder-state";
const ART_DIRECTION_DEFAULTS: ArtDirectionSelections = {
  "home.founder-authority": "a",
  "home.team-continuity": "a",
  "home.technology-purpose": "a",
  "home.heritage-story": "a",
};
const ART_DIRECTION_OPTIONS: Record<ArtDirectionSectionId, { label: string; a: { name: string; rationale: string }; b: { name: string; rationale: string } }> = {
  "home.founder-authority": { label: "Founder authority", a: { name: "Portrait-ready arch", rationale: "Asymmetric authority with governed space for a future genuine portrait." }, b: { name: "Credential ledger", rationale: "Editorial evidence rhythm with a narrow future-media rail." } },
  "home.team-continuity": { label: "Team continuity", a: { name: "Connected rooms", rationale: "Warm interlocking spaces express joined-up human care." }, b: { name: "Continuity pathway", rationale: "A calmer horizontal handover line makes continuity explicit." } },
  "home.technology-purpose": { label: "Technology purpose", a: { name: "Luminous precision", rationale: "Restrained focus light makes technology clarify one decision." }, b: { name: "Purpose ledger", rationale: "An editorial instrument field prioritises clinical purpose over gadgets." } },
  "home.heritage-story": { label: "Heritage story", a: { name: "Architectural threshold", rationale: "Measured doorway proportions evoke place without inventing documentary imagery." }, b: { name: "Archival folio", rationale: "Typographic chronology geometry carries heritage without false dates or photographs." } },
};
...
export function Atelier() {
  ...
  const [artDirections, setArtDirections] = useState<ArtDirectionSelections>(ART_DIRECTION_DEFAULTS);
  const [artDirectionOpen, setArtDirectionOpen] = useState(false);
  const [artDirectionSection, setArtDirectionSection] = useState<ArtDirectionSectionId>("home.founder-authority");
  ...
  const [studioEntryDomain, setStudioEntryDomain] = useState<GenerationDomain>("webpage");
  const [activeProposal, setActiveProposal] = useState<DesignProposal | null>(null);
  const [proposalHistory, setProposalHistory] = useState<Array<DesignProposal | null>>([]);
  const [compareGolden, setCompareGolden] = useState(false);
  ...
  const brief = useMemo(() => ({
    ...
    homepageArtDirection: { schema: "champagne.atelier.homepage-art-direction.v1", selections: artDirections, contentAuthority: "UNCHANGED", heroAuthorityConflict: "UNRESOLVED_PRESERVED", productionBinding: false },
    brandDecision,
    mediaLens,
  ...
  useEffect(() => {
    try {
      ...
      if (!saved) return;
      const parsed = JSON.parse(saved) as { brandDecision?: BrandDecision; decisions?: Record<string, Decision>; artDirections?: ArtDirectionSelections };
      ...
      if (parsed.artDirections) setArtDirections((current) => ({ ...current, ...parsed.artDirections }));
  ...
  useEffect(() => {
    ...
    try { window.localStorage.setItem(ATELIER_STORAGE_KEY, JSON.stringify({ schema: "champagne.atelier.founder-state.v1", brandDecision, decisions, artDirections })); } catch { /* Export remains the durable fallback. */ }
  }, [artDirections, brandDecision, decisions, founderStateRestored]);
  ...
  const selectSection = (id: string) => {
    setSelected(id);
    window.requestAnimationFrame(() => previewRef.current?.querySelector<HTMLIFrameElement>("iframe")?.contentDocument?.querySelector<HTMLElement>(`[data-semantic-id="${id}"]`)?.scrollIntoView({ block: "start", behavior: "smooth" }));
  };
  const scrollPreviewTop = () => previewRef.current?.querySelectorAll<HTMLIFrameElement>("iframe").forEach((frame) => frame.contentWindow?.scrollTo({ top: 0, behavior: "smooth" }));
  ...
  const setRoomUrl = useCallback((room?: string) => { const url = new URL(window.location.href); if (room) url.searchParams.set("room", room); else url.searchParams.delete("room"); window.history.replaceState(null, "", url); }, []);
  const closeFounderDesignStudio = useCallback(() => { setFounderDesignStudioOpen(false); setRoomUrl(); }, [setRoomUrl]);
  const openFounderDesignStudio = useCallback((domain: GenerationDomain = "webpage") => { setStudioEntryDomain(domain); setFounderDesignStudioOpen(true); setRoomUrl(domain === "concierge" ? "concierge-ux" : "founder-design-studio"); }, [setRoomUrl]);
  const previewProposal = useCallback((proposal: DesignProposal) => { setProposalHistory((items) => [...items, activeProposal]); setActiveProposal(proposal); setCompareGolden(false); setFounderDesignStudioOpen(false); setRoomUrl(); if (proposal.domain === "webpage" && proposal.pageKey) { setPage(proposal.pageKey); const targetSection = sections[proposal.pageKey].find((item) => item.id === proposal.semanticOwner); setSelected(targetSection?.id ?? sections[proposal.pageKey][0].id); } }, [activeProposal, sections, setRoomUrl]);
  const returnToGolden = useCallback(() => { setProposalHistory((items) => [...items, activeProposal]); setActiveProposal(null); setCompareGolden(false); }, [activeProposal]);
  const undoProposal = useCallback(() => setProposalHistory((items) => { const next = [...items]; setActiveProposal(next.pop() ?? null); return next; }), []);
  useEffect(() => {
    const room = new URL(window.location.href).searchParams.get("room");
    if (!room) return;
    if (room === "founder-design-studio" || room === "concierge-ux") { openEditor("home"); setStudioEntryDomain(room === "concierge-ux" ? "concierge" : "webpage"); setFounderDesignStudioOpen(true); }
    else if (room === "brand-workshop") setView("brand");
    else if (room === "media-studio") { openEditor("home"); setConvergenceOpen(true); }
    else setRoomUrl();
  // Deep-link state is intentionally interpreted only once on entry.
  ...
  const previewState = (time: StudioTime, proposal: DesignProposal | null = activeProposal): AtelierPreviewState => ({ page, sections: current, selected, cleanPreview, time, brandDecision, canvasStyle, closingPlacement, closingTreatment, artDirections, experiment: proposal ? { id: proposal.id, domain: proposal.domain, semanticOwner: proposal.semanticOwner, family: proposal.family, scope: proposal.scope, targetKind: proposal.targetKind, pageKey: proposal.pageKey, componentId: proposal.componentId } : null });
  ...
  return <main className="dl4-app dl45-app" data-clean-preview={cleanPreview}>
    <header className="dl4-topbar"><button className="dl4-home-button" onClick={() => setView("welcome")}><strong>Champagne Atelier</strong><span>Studio home</span></button><nav className="dl43-workspace-nav" aria-label="Atelier workspace"><button onClick={() => { setRoomUrl("brand-workshop"); setView("brand"); }}>Brand workshop</button><button aria-current="page">Page atelier</button><button onClick={() => setArtDirectionOpen(true)}>Art direction</button><button onClick={() => setConvergenceOpen(true)}>Experience layers</button><button className="dl49-launch" onClick={() => openFounderDesignStudio("webpage")}>Generate &amp; explore</button></nav><label><span>Page</span><select aria-label="Page" value={page} onChange={(event) => changePage(event.target.value as AtelierPageKey)}>{Object.entries(PAGE_NAMES).map(([key, name]) => <option key={key} value={key}>{name}</option>)}</select></label><div className="dl45-preview-actions"><button aria-pressed={cleanPreview} onClick={() => { if (cleanPreview && fullscreenMode === "FALLBACK") setFullscreenMode("OFF"); setCleanPreview((value) => !value); }}>{cleanPreview ? "Return to studio" : "Clean preview"}</button><button onClick={toggleFullscreen}>Fullscreen</button></div><div className="dl4-actions"><span>Simulation only · production binding off</span><button onClick={exportBrief}>Export governed brief</button></div></header>

    <aside className="dl4-pages"><div className="dl4-panel-heading"><h2>Pages</h2><button aria-label="Propose a new page" onClick={openProposal}>＋</button></div>{(Object.keys(PAGE_NAMES) as AtelierPageKey[]).map((key) => <button className="dl4-page" aria-current={page === key ? "page" : undefined} key={key} onClick={() => changePage(key)}><span>{PAGE_NAMES[key]}</span><small>{visibleAtelierSections(ATELIER_CONTENT_PAGES[key]).length} visible · {ATELIER_CONTENT_PAGES[key].sections.length} jobs</small></button>)}<div className="dl4-content-status" data-state={pageContract.bundleStatus}><strong>{page === "home" ? "Real Home Content Bundle connected" : page === "implants" ? "Accepted Implant Content Bundle connected" : "Lab seed copy"}</strong><p>{page === "home" ? "12 visible chapters · FACT BLOCKED · composition testing only. Patient evidence is fully omitted." : page === "implants" ? "12 enabled governed chapters · clinical and fact review pending · not publishable. Case evidence remains off." : "The section jobs are authoritative. The prose remains temporary until its approved content bundle arrives."}</p></div><div className="dl4-panel-heading"><h2>Page flow</h2><button aria-label="Back to page top" onClick={scrollPreviewTop}>↑</button></div><ol className="dl4-layers">{current.map((item, index) => <li key={item.id}><button aria-current={selected === item.id} onClick={() => selectSection(item.id)}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{item.label}</strong><small>{item.locked ? "Canonical · protected" : item.archiveId ? "Archive design placed" : item.contentState === "GOVERNED_CONTENT_CLINICAL_AND_FACT_BLOCKED" ? "Governed copy · review blocked" : item.contentState === "CONTENT_BUNDLE_V1_FACT_BLOCKED" ? "Real copy · fact blocked" : "Lab seed copy"}</small></div></button></li>)}</ol><div className="dl4-left-actions"><button onClick={() => setDrawer("archive")}>Explore existing designs</button><button onClick={openProposal}>✦ Create something new</button></div></aside>

    <section className="dl4-workspace">{activeProposal ? <div className="dl410-experiment-bar" data-domain={activeProposal.domain}><div><span>Current experiment</span><strong>LAB GENERATED PROPOSAL · {activeProposal.id}</strong><small>Not Golden baseline · productionBinding=false</small></div><div><button onClick={undoProposal} disabled={!proposalHistory.length}>Undo last design action</button><button aria-pressed={compareGolden} onClick={() => setCompareGolden((value) => !value)}>Golden vs candidate</button><button onClick={() => openFounderDesignStudio(activeProposal.domain)}>Refine in Studio</button><button onClick={returnToGolden}>Back to Golden</button></div></div> : <div className="dl410-golden-bar"><strong>Golden baseline</strong><span>No generated experiment is applied.</span><button onClick={() => openFounderDesignStudio("webpage")}>Generate more</button><button onClick={() => openFounderDesignStudio("concierge")}>Concierge UX Studio</button></div>}<div className="dl4-context"><span>{PAGE_NAMES[page]} · real semantic flow</span><strong>{resolvedViewport.width} × {resolvedViewport.height} · {displayScale}% display</strong><span>{pageContract.primaryQuestion}</span></div><div className="dl4-scroll dl45-preview-stage" ref={previewRef} data-device={device} data-orientation={orientation} data-viewport-width={resolvedViewport.width} data-viewport-height={resolvedViewport.height} data-display-scale={displayScale} data-device-frame={deviceFrame} data-studio-time={studioTime} data-time-canon={TEMPORAL_SIMULATIONS[studioTime].canon} data-time-runtime={TEMPORAL_SIMULATIONS[studioTime].heroRuntime} data-time-scope={TEMPORAL_SIMULATIONS[studioTime].scope} data-fullscreen-mode={fullscreenMode} data-brand-territory={brandDecision.territory} data-brand-accent={brandDecision.accent} data-brand-type={brandDecision.typography} data-brand-rhythm={brandDecision.rhythm} data-persian-candidate={persianChoice.id} data-porcelain-candidate={porcelainChoice.id} data-comparing={Boolean(compareTime || compareGolden)}>{compareGolden && activeProposal ? <div className="dl410-compare-label">Golden baseline</div> : null}{compareGolden && activeProposal ? <TruthfulPreviewFrame viewport={resolvedViewport} displayScale={displayScale} title={`${PAGE_NAMES[page]} Golden baseline`} state={previewState(studioTime, null)} /> : null}{compareGolden && activeProposal ? <div className="dl410-compare-label">Candidate {activeProposal.id}</div> : null}<TruthfulPreviewFrame viewport={resolvedViewport} displayScale={displayScale} title={`${PAGE_NAMES[page]} ${device} preview`} state={previewState(studioTime)} />{compareTime ? <TruthfulPreviewFrame viewport={resolvedViewport} displayScale={displayScale} title={`${PAGE_NAMES[page]} comparison preview`} state={previewState(compareTime)} /> : null}{fullscreenMode === "FALLBACK" ? <button className="dl451-fallback-exit" onClick={() => { setFullscreenMode("OFF"); setCleanPreview(false); }}>Exit viewport-filling preview</button> : null}</div></section>

    <aside className="dl4-inspector"><div className="dl4-panel-heading"><h2>Shape the look</h2><span>Live on canvas</span></div><button className="dl43-brand-chip" onClick={() => setView("brand")}><span>Working Brand DNA</span><strong>{brandDecision.territory.replaceAll("-", " ")}</strong><small>{brandDecision.typography} · {brandDecision.rhythm}</small></button><div className="dl4-material-tabs"><button aria-pressed={material === "persian"} onClick={() => setMaterial("persian")}>Persian Velvet Blue</button><button aria-pressed={material === "porcelain"} onClick={() => setMaterial("porcelain")}>Porcelain</button></div>{material === "persian" ? <MaterialPicker items={PERSIAN_CANDIDATES} value={persian} onChange={setPersian} colour={(item) => colour(item.canvas)} /> : <MaterialPicker items={PORCELAIN_CANDIDATES} value={porcelain} onChange={setPorcelain} colour={(item) => colour(item.base)} />}{active.id === "home.closing-invitation" ? <ArchitecturalClosingControls placement={closingPlacement} treatment={closingTreatment} onPlacement={setClosingPlacement} onTreatment={setClosingTreatment} /> : null}<section className="dl4-active"><span>Selected on page</span><h2>{active.label}</h2><p>{active.job}</p><small>{active.contentState === "GOVERNED_CONTENT_CLINICAL_AND_FACT_BLOCKED" ? "Accepted Implant Content Bundle · CLINICAL + FACT BLOCKED · not publishable" : active.contentState === "CONTENT_BUNDLE_V1_FACT_BLOCKED" ? "Real Content Bundle · FACT BLOCKED · not publishable" : "Temporary Lab seed copy · semantic job preserved"}</small>{active.id in ART_DIRECTION_OPTIONS ? <button className="dl47-compare-trigger" onClick={() => { setArtDirectionSection(active.id as ArtDirectionSectionId); setArtDirectionOpen(true); }}>Compare this chapter</button> : null}{!active.locked && <div><button onClick={() => move(-1)}>Move up</button><button onClick={() => move(1)}>Move down</button><button onClick={() => setDrawer("archive")}>Existing designs</button><button onClick={openProposal}>✦ New proposal</button></div>}</section><MediaLens lens={mediaLens} onOpen={() => setConvergenceOpen(true)} /><section className="dl4-decisions"><h2>Your decision</h2><div>{(["love", "keep", "maybe", "reject"] as Decision[]).map((choice) => <button key={choice} aria-pressed={decisions[active.id] === choice} onClick={() => setDecisions((items) => ({ ...items, [active.id]: choice }))}>{choice}</button>)}</div></section><section className="dl4-ask"><h2>Ask Atelier</h2><p>Describe the feeling or page change you want. It will become a truthful Lab proposal.</p><textarea value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder="For example: make this warmer, more editorial and less formal…" /><button onClick={openProposal}>Turn this into a proposal</button>{proposals.length ? <small>{proposals.length} saved proposal{proposals.length === 1 ? "" : "s"} in this brief</small> : null}</section></aside>
    ...
    {founderDesignStudioOpen ? <FounderDesignStudio key={`${studioEntryDomain}-${page}`} initialDomain={studioEntryDomain} initialPage={page} pages={ATELIER_CONTENT_PAGES} selectedSectionId={selected} onClose={closeFounderDesignStudio} onGovernedChange={setFounderDesignStudio} onPreview={previewProposal} onReturnGolden={returnToGolden} activeProposalId={activeProposal?.id ?? null} /> : null}
  </main>;
}
...
function ArtDirectionRoom({ sectionId, selections, decisions, onSection, onSelect, onDecision, onClose }: { sectionId: ArtDirectionSectionId; selections: ArtDirectionSelections; decisions: Record<string, Decision>; onSection: (value: ArtDirectionSectionId) => void; onSelect: (sectionId: ArtDirectionSectionId, variant: ArtDirectionVariant) => void; onDecision: (sectionId: ArtDirectionSectionId, variant: ArtDirectionVariant, decision: Decision) => void; onClose: () => void }) {
  const option = ART_DIRECTION_OPTIONS[sectionId];
  return <div className="dl47-room-backdrop"><section className="dl47-room" role="dialog" aria-modal="true" aria-labelledby="art-direction-heading"><header><div><span>Homepage art-direction room</span><h2 id="art-direction-heading">Compare meaning, not decoration.</h2><p>One chapter at a time · governed copy unchanged · missing media remains honest.</p></div><button onClick={onClose}>Close room</button></header><nav aria-label="Art-direction chapters">{(Object.keys(ART_DIRECTION_OPTIONS) as ArtDirectionSectionId[]).map((id) => <button key={id} aria-current={id === sectionId ? "page" : undefined} onClick={() => onSection(id)}>{ART_DIRECTION_OPTIONS[id].label}<small>{selections[id] === "a" ? ART_DIRECTION_OPTIONS[id].a.name : ART_DIRECTION_OPTIONS[id].b.name}</small></button>)}</nav><div className="dl47-comparison" data-section={sectionId}>{(["a", "b"] as ArtDirectionVariant[]).map((variant) => { const treatment = option[variant]; const selected = selections[sectionId] === variant; const decisionKey = `${sectionId}:${variant}`; return <article key={variant} data-variant={variant} data-selected={selected}><div className="dl47-comparison-preview" aria-hidden="true"><span>{variant.toUpperCase()}</span><i /><i /><i /><strong>{option.label}</strong></div><div className="dl47-comparison-copy"><span>Treatment {variant.toUpperCase()}</span><h3>{treatment.name}</h3><p>{treatment.rationale}</p><button aria-pressed={selected} onClick={() => onSelect(sectionId, variant)}>{selected ? "Selected for Homepage" : "Use this treatment"}</button><div aria-label={`${treatment.name} decision`}>{(["love", "keep", "maybe", "reject"] as Decision[]).map((decision) => <button key={decision} aria-pressed={decisions[decisionKey] === decision} onClick={() => onDecision(sectionId, variant, decision)}>{decision}</button>)}</div></div></article>; })}</div><footer><span>Founder working decisions save locally and export in the governed brief.</span><strong>productionBinding=false</strong></footer></section></div>;
}

function TruthfulPreviewFrame({ viewport, displayScale, title, state }: { viewport: { width: number; height: number }; displayScale: number; title: string; state: AtelierPreviewState }) {
  const frameRef = useRef<HTMLIFrameElement>(null);
  const sendState = useCallback(() => frameRef.current?.contentWindow?.postMessage({ type: "CHAMPAGNE_ATELIER_PREVIEW_STATE", state }, window.location.origin), [state]);
  useEffect(() => {
    const receive = (event: MessageEvent) => {
      if (event.origin === window.location.origin && event.source === frameRef.current?.contentWindow && event.data?.type === "CHAMPAGNE_ATELIER_PREVIEW_READY") sendState();
    };
  ...
  return <div className="dl45-preview-frame dl46-preview-frame" style={{ width: viewport.width, height: viewport.height, transform: `scale(${displayScale / 100})` }}>
    <iframe ref={frameRef} className="dl46-preview-viewport" title={title} src="/champagne/design-lab/preview" onLoad={sendState} />
  </div>;
}
