...
const SEED: Record<PageKey, Section[]> = {
  home: [
    ...
    { id: "architectural-closing", label: "Architectural closing", tone: "persian", title: "Exceptional care. Enduring confidence.", copy: "A replaceable architectural closing awaiting the genuine St Mary’s House photograph." },
...
export function Atelier({ heroes }: { heroes: Record<PageKey, ReactNode> }) {
  ...
  const brief = useMemo(() => ({ schema: "champagne.atelier.handoff.v3", page, viewport, materials: { persian: persianChoice, porcelain: porcelainChoice }, sections: current.map((item, order) => ({ ...item, order })), architecturalClosing: { ...architecturalClosingContract, selectedPlacement: closingPlacement, selectedTreatment: closingTreatment }, founderInstruction: instruction, productionBinding: false, approval: "FOUNDER_REVIEW_REQUIRED" }), [page, viewport, persianChoice, porcelainChoice, current, instruction, closingPlacement, closingTreatment]);
...
function ArchitecturalClosing({ placement, treatment }: { placement: ClosingPlacement; treatment: ClosingTreatment }) {
  return <div className="dl4-architectural" data-placement={placement} data-treatment={treatment}>
    <img src={architecturalClosingConcept.src} alt="Fictional architectural entrance used only to preview the St Mary's House closing composition" />
    <div className="dl4-architectural-shade" />
    <div className="dl4-architectural-copy"><span>St Mary’s House · architectural closing study</span><h2>Exceptional care.<br />Enduring confidence.</h2><i aria-hidden="true" /><p>Final version awaits your genuine entrance photograph.</p><button>Book a consultation</button></div>
    <div className="dl4-concept-label">Lab proposal · fictional architecture · not St Mary’s House</div>
    {placement === "FULL_FOOTER" ? <footer className="dl4-architectural-footer"><strong>St Mary’s House Dental Care</strong><nav aria-label="Architectural footer preview"><span>Treatments</span><span>Our approach</span><span>The practice</span><span>Contact</span></nav></footer> : null}
  </div>;
}

function ArchitecturalClosingControls({ placement, treatment, onPlacement, onTreatment }: { placement: ClosingPlacement; treatment: ClosingTreatment; onPlacement: (value: ClosingPlacement) => void; onTreatment: (value: ClosingTreatment) => void }) {
  return <section className="dl4-closing-controls"><span>Architectural closing</span><h2>Choose its role and finish</h2><div className="dl4-segmented"><button aria-pressed={placement === "PRE_FOOTER_CLOSING_SECTION"} onClick={() => onPlacement("PRE_FOOTER_CLOSING_SECTION")}>Above footer</button><button aria-pressed={placement === "FULL_FOOTER"} onClick={() => onPlacement("FULL_FOOTER")}>Full footer</button></div><div className="dl4-treatment-list"><button aria-pressed={treatment === "PERSIAN_ARCHITECTURAL"} onClick={() => onTreatment("PERSIAN_ARCHITECTURAL")}><i data-swatch="persian" />Persian architectural</button><button aria-pressed={treatment === "PORCELAIN_GALLERY"} onClick={() => onTreatment("PORCELAIN_GALLERY")}><i data-swatch="porcelain" />Porcelain gallery</button><button aria-pressed={treatment === "GILDED_BRAND_GOLD"} onClick={() => onTreatment("GILDED_BRAND_GOLD")}><i data-swatch="gold" />Gilded brand gold</button></div><p>The genuine photograph will replace only the media layer. The regeneration and provenance rules travel inside every exported handoff.</p></section>;
}
