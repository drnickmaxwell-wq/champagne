...
export function Atelier() {
  ...
  const previewProposal = useCallback((proposal: DesignProposal) => { setProposalHistory((items) => [...items, activeProposal]); setActiveProposal(proposal); setCompareGolden(false); setFounderDesignStudioOpen(false); setRoomUrl(); if (proposal.domain === "webpage" && proposal.semanticOwner.startsWith("home.")) setSelected(proposal.semanticOwner); }, [activeProposal, setRoomUrl]);
  ...
  return <main className="dl4-app dl45-app" data-clean-preview={cleanPreview}>
    ...
    {founderDesignStudioOpen ? <FounderDesignStudio key={studioEntryDomain} initialDomain={studioEntryDomain} onClose={closeFounderDesignStudio} onGovernedChange={setFounderDesignStudio} onPreview={previewProposal} activeProposalId={activeProposal?.id ?? null} /> : null}
  </main>;
}
