...
export function AtelierPreviewDocument({ heroes }: { heroes: Record<AtelierPageKey, ReactNode> }) {
  ...
  return <main className="dl46-preview-document" data-preview-ready="true" data-production-binding="false">
    <article
      className="dl4-canvas dl45-canvas dl46-canvas"
    ...
    >
      {state.sections.map(item => <section key={item.id} data-semantic-id={item.id} data-content-state={item.contentState} data-tone={item.tone} data-treatment={item.id === "home.closing-invitation" ? state.closingTreatment : undefined} className={state.selected === item.id ? "is-selected" : ""}>
