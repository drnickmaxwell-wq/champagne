...
function ImplantChapterRail({ sections, family }: { sections: PlacedSection[]; family: NonNullable<AtelierPreviewState["experiment"]>["family"] }) {
  return <nav className="dl412-chapter-rail" aria-label="Implant page chapters" data-family={family}><strong>Dental Implants</strong><ol>{sections.map((section, index) => <li key={section.id}><a href={`#${section.id}`}><span>{String(index + 1).padStart(2, "0")}</span>{section.label}</a></li>)}</ol><small>Founder visual study · publication blocked</small></nav>;
}
...
function ContentSection({ item, page, family }: { item: PlacedSection; page: AtelierPageKey; family: NonNullable<AtelierPreviewState["experiment"]>["family"] | null }) { return <div id={item.id} className={`dl4-native dl44-content dl44-${item.id.replaceAll(".", "-")}`}><div className="dl411-copy"><span>{item.label}</span><h2>{item.title}</h2>{item.copy ? item.copy.split("\n\n").map(paragraph => <p key={paragraph}>{paragraph}</p>) : null}{item.pathways ? <div className="dl44-pathways">{item.pathways.map((pathway, index) => <a href={pathway.href} key={pathway.href}><b>{String(index + 1).padStart(2, "0")}</b><strong>{pathway.label}</strong><span>{pathway.description}</span><i>Explore →</i></a>)}</div> : null}{item.steps ? <ol className="dl44-steps">{item.steps.map((step, index) => <li key={step.label}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{step.label}</strong><p>{step.copy}</p></div></li>)}</ol> : null}{item.componentCards ? <div className="dl411-components">{item.componentCards.map((component, index) => <article key={component.answerObjectId}><span>{String(index + 1).padStart(2, "0")}</span><h3>{component.label}</h3><p>{component.copy}</p></article>)}</div> : null}{item.faqs ? <div className="dl44-faqs">{item.faqs.map(faq => <details key={faq.question}><summary>{faq.question}</summary><p>{faq.answer}</p></details>)}</div> : null}<SectionGeometry id={item.id} />{item.transcript ? <details className="dl411-transcript"><summary>Read the complete 3D text alternative</summary><p>{item.transcript}</p></details> : null}{item.ctas?.length ? <div className="dl44-ctas">{item.ctas.map(cta => <a key={`${cta.href}-${cta.label}`} href={cta.href}>{cta.label}</a>)}</div> : null}</div>{item.modelSlot ? <ImplantExhibitionSlot item={item} family={family} /> : item.contentMediaSlotIds?.length || item.mediaSlot ? <MediaComposition item={item} family={family} /> : null}{item.governance ? <details className={`dl411-governance ${page === "implants" && item.id === "implants.faq-sources" ? "dl412-evidence-host" : ""}`}><summary>{item.id === "implants.faq-sources" ? "Sources and review notes" : "Content authority and review state"}</summary><dl><div><dt>Bundle</dt><dd>{item.governance.bundleId}</dd></div><div><dt>Clinical/fact state</dt><dd>{item.reviewState}</dd></div><div><dt>Publication</dt><dd>{item.governance.publicationMaturity}</dd></div><div><dt>Answer Objects</dt><dd>{item.answerObjectIds?.join(", ")}</dd></div><div><dt>Sources</dt><dd>{item.sourceGroupIds?.join(", ")}</dd></div></dl></details> : null}</div>; }

function MediaComposition({ item, family }: { item: PlacedSection; family: NonNullable<AtelierPreviewState["experiment"]>["family"] | null }) { return <figure className="dl411-media-slot dl412-media-composition" data-family={family ?? "golden"}><div aria-hidden="true"><i /><i /><i /></div><figcaption><span>Real media required · provenance-safe composition</span><strong>{item.contentMediaSlotIds?.[0] ?? item.mediaSlot}</strong><p>{item.capabilityOffBehavior ?? "Deliberate text-led fallback until provenance-complete media exists."}</p><small>Real practice photography required · no synthetic patient or practice image</small></figcaption></figure>; }

function ImplantExhibitionSlot({ item, family }: { item: PlacedSection; family: NonNullable<AtelierPreviewState["experiment"]>["family"] | null }) { return <figure className="dl411-3d-slot dl412-3d-exhibition" data-family={family ?? "golden"}><div className="dl412-plinth" aria-hidden="true"><i /><i /><i /></div><figcaption><span>Governed 3D exhibition</span><strong>Final Champagne implant asset pending</strong><p>Founder-rejected T0.1 geometry is not integrated. This architectural plinth reserves the visual role without pretending to be the clinical model.</p><small>{item.modelSlot} · poster/static/fallback seam preserved</small></figcaption></figure>; }
