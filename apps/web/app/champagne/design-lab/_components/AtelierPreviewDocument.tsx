...
export function AtelierPreviewDocument({ heroes }: { heroes: Record<AtelierPageKey, ReactNode> }) {
  ...
  return <main className="dl46-preview-document" data-preview-ready="true" data-production-binding="false">
    <article
      ...
      data-whole-page-proposal={state.experiment?.domain === "webpage" && state.experiment.targetKind === "page" && state.experiment.pageKey === state.page && state.page !== "implants" ? state.experiment.family : undefined}
      data-r414-canonical={state.page === "implants" && state.experiment?.domain === "webpage" && state.experiment.targetKind === "page" ? "implant-page" : undefined}
      style={state.canvasStyle}
    >
      {state.page === "implants" && state.experiment?.targetKind === "page" ? <ImplantChapterRail sections={state.sections} family={state.experiment.family} /> : null}
      {state.sections.map((item, index) => <section key={item.id} data-semantic-id={item.id} data-content-state={item.contentState} data-tone={item.tone} data-chapter={String(index + 1).padStart(2, "0")} data-art-direction={state.artDirections[item.id] ?? "a"} data-treatment={item.id === "home.closing-invitation" ? state.closingTreatment : undefined} data-lab-proposal={state.experiment?.domain === "webpage" && state.experiment.targetKind !== "page" && state.experiment.semanticOwner === item.id ? state.experiment.family : undefined} data-lab-component={state.experiment?.targetKind === "component" && state.experiment.semanticOwner === item.id ? state.experiment.componentId ?? undefined : undefined} data-bundle-hash={item.governance?.bundleContentHash} data-publication-maturity={item.governance?.publicationMaturity} className={state.selected === item.id ? "is-selected" : ""}>
        {item.locked || (state.page === "implants" && item.id === "implants.hero") ? heroes[state.page]
          : item.id === "home.closing-invitation" ? <ArchitecturalClosing item={item} placement={state.closingPlacement} treatment={state.closingTreatment} />
            : item.archiveId ? <figure className="dl4-placed"><img src={assetFor(item.archiveId)} alt={`${item.label} visual proposal`} /><figcaption><span>Archive proposal</span></figcaption></figure>
              : <ContentSection item={item} page={state.page} family={state.experiment?.targetKind === "page" ? state.experiment.family : null} />}
...
function ImplantChapterRail({ sections, family }: { sections: PlacedSection[]; family: NonNullable<AtelierPreviewState["experiment"]>["family"] }) {
  const pageSections = sections.filter(section => section.id !== "implants.hero");
  return <nav className="dl412-chapter-rail dl414-chapter-rail" aria-label="Implant page chapters" data-family={family}><strong>Explore the page</strong><ol>{pageSections.map((section, index) => <li key={section.id}><a href={`#${section.id}`}><span>{String(index + 1).padStart(2, "0")}</span>{section.label}</a></li>)}</ol><small>Canonical Brand DNA · Founder review</small></nav>;
}
...
const PROMINENT_IMPLANT_MEDIA_SEAMS = new Set(["implants.planning", "implants.aftercare"]);

function ContentSection({ item, page, family }: { item: PlacedSection; page: AtelierPageKey; family: NonNullable<AtelierPreviewState["experiment"]>["family"] | null }) { const showMedia = page !== "implants" || PROMINENT_IMPLANT_MEDIA_SEAMS.has(item.id); return <div id={item.id} className={`dl4-native dl44-content dl44-${item.id.replaceAll(".", "-")}`}><div className="dl411-copy"><span>{item.label}</span><h2>{item.title}</h2>{item.copy ? item.copy.split("\n\n").map(paragraph => <p key={paragraph}>{paragraph}</p>) : null}{item.pathways ? <div className="dl44-pathways">{item.pathways.map((pathway, index) => <a href={pathway.href} key={pathway.href}><b>{String(index + 1).padStart(2, "0")}</b><strong>{pathway.label}</strong><span>{pathway.description}</span><i>Explore →</i></a>)}</div> : null}{item.steps ? <ol className="dl44-steps">{item.steps.map((step, index) => <li key={step.label}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{step.label}</strong><p>{step.copy}</p></div></li>)}</ol> : null}{item.componentCards ? <div className="dl411-components">{item.componentCards.map((component, index) => <article key={component.answerObjectId}><span>{String(index + 1).padStart(2, "0")}</span><h3>{component.label}</h3><p>{component.copy}</p></article>)}</div> : null}{item.faqs ? <div className="dl44-faqs">{item.faqs.map(faq => <details key={faq.question}><summary>{faq.question}</summary><p>{faq.answer}</p></details>)}</div> : null}<SectionGeometry id={item.id} />{item.transcript ? <details className="dl411-transcript"><summary>Read the complete 3D text alternative</summary><p>{item.transcript}</p></details> : null}{item.ctas?.length ? page === "implants" && ["implants.assessment-factors", "implants.next-step"].includes(item.id) ? <ImplantCtaContext item={item} /> : <div className="dl44-ctas">{item.ctas.map(cta => <a key={`${cta.href}-${cta.label}`} href={cta.href}>{cta.label}</a>)}</div> : null}</div>{item.modelSlot ? <ImplantExhibitionSlot item={item} family={family} /> : showMedia && (item.contentMediaSlotIds?.length || item.mediaSlot) ? <MediaComposition item={item} family={family} /> : null}{item.governance ? <details className={`dl411-governance ${page === "implants" && item.id === "implants.faq-sources" ? "dl412-evidence-host" : ""}`}><summary>{item.id === "implants.faq-sources" ? "Sources and review notes" : "Content authority and review state"}</summary><dl><div><dt>Bundle</dt><dd>{item.governance.bundleId}</dd></div><div><dt>Clinical/fact state</dt><dd>{item.reviewState}</dd></div><div><dt>Publication</dt><dd>{item.governance.publicationMaturity}</dd></div><div><dt>Answer Objects</dt><dd>{item.answerObjectIds?.join(", ")}</dd></div><div><dt>Sources</dt><dd>{item.sourceGroupIds?.join(", ")}</dd></div></dl></details> : null}</div>; }

type ImplantCtaFamily = "wave-luxe" | "porcelain-editorial" | "signature-wave-motion";
const IMPLANT_CTA_FAMILIES: Array<{ id: ImplantCtaFamily; label: string }> = [
  { id: "wave-luxe", label: "A · Wave Luxe" },
  { id: "porcelain-editorial", label: "B · Porcelain Editorial" },
  { id: "signature-wave-motion", label: "C · Signature Wave Motion" },
];

function ImplantCtaContext({ item }: { item: PlacedSection }) {
  const [selected, setSelected] = useState<ImplantCtaFamily>(item.id === "implants.next-step" ? "signature-wave-motion" : "wave-luxe");
  return <aside className="dl414-cta-context" data-cta-family={selected}>
    <div className="dl414-cta-preview"><span>CTA in this chapter</span><div className="dl44-ctas">{item.ctas?.map(cta => <a key={`${cta.href}-${cta.label}`} href={cta.href}>{cta.label}</a>)}</div></div>
    <fieldset><legend>Compare the treatment here</legend>{IMPLANT_CTA_FAMILIES.map(option => <button key={option.id} type="button" aria-pressed={selected === option.id} onClick={() => setSelected(option.id)}>{option.label}</button>)}</fieldset>
  </aside>;
}

function MediaComposition({ item, family }: { item: PlacedSection; family: NonNullable<AtelierPreviewState["experiment"]>["family"] | null }) { return <figure className="dl411-media-slot dl412-media-composition dl414-media-seam" data-family={family ?? "golden"}><div aria-hidden="true"><i /><i /><i /></div><figcaption><span>Real media required · provenance-safe composition</span><strong>{item.contentMediaSlotIds?.[0] ?? item.mediaSlot}</strong><p>{item.capabilityOffBehavior ?? "Deliberate text-led fallback until provenance-complete media exists."}</p><small>Awaiting approved Champagne imagery · no synthetic patient or practice image</small></figcaption></figure>; }

function ImplantExhibitionSlot({ item, family }: { item: PlacedSection; family: NonNullable<AtelierPreviewState["experiment"]>["family"] | null }) { return <figure className="dl411-3d-slot dl412-3d-exhibition dl414-3d-seam" data-family={family ?? "golden"}><div className="dl414-wave-stage" aria-hidden="true"><i /><i /><i /><b /><b /></div><figcaption><span>Governed 3D / educational seam</span><strong>Champagne medical art direction pending</strong><p>The future approved model or editorial still can enter this purposeful stage without changing the chapter. Founder-rejected T0.1 geometry is not integrated. Rejected procedural anatomy and IMPLANT-ED-01–05 are not integrated.</p><small>{item.modelSlot} · static poster · interactive model · full text alternative</small></figcaption></figure>; }
