...
export function RegistryBrowser({ room = "headers" }: { room?: string }) {
  ...
  const compare = registry.items.find((item) => item.id === compareId);
  ...
  return <section className="dl-browser" aria-labelledby="component-browser-heading">
    ...
    <article className="dl-component-stage"><div className="dl-component-copy"><p>{groupLabel}</p><h2>{current.title}</h2><p>{current.purpose || "A recovered visual direction for Champagne."}</p></div><div className="dl-component-image">{imageFailed ? <span>Preview temporarily unavailable</span> : <img src={previewUrl(current)} alt={`${current.title} visual design`} onError={() => setImageFailed(true)} />}</div></article>
    {compare ? <section className="dl-compare" aria-label="Comparison design"><div><h2>Compare with {compare.title}</h2><button type="button" onClick={() => setCompareId(null)}>Close comparison</button></div><img src={previewUrl(compare)} alt={`${compare.title} comparison visual`} /></section> : null}
    <div className="dl-choice-actions"><button type="button" aria-pressed={selected.includes(current.id)} onClick={() => toggle(current.id)}>♡ {selected.includes(current.id) ? "Shortlisted" : "Add to shortlist"}</button><button type="button" onClick={() => setCompareId(items[(index + 1) % items.length]?.id)}>Compare with next</button><a href="/champagne/design-lab#room-11">Add choices in Room 11</a></div>
