...
export function ExperienceRooms({ onClose }: { onClose: () => void }) {
  ...
  return <div className="dl44-rooms" role="dialog" aria-modal="true" aria-labelledby="experience-room-heading">
    <header className="dl44-room-head"><div><span>R4.4 cross-lane convergence</span><h2 id="experience-room-heading">Experience rooms</h2><p>One Champagne experience, with each lane’s authority still visible.</p></div><button onClick={onClose}>Close</button></header>
...
function Journey({ step, onStep, onThreeD }: { step: number; onStep: (step: number) => void; onThreeD: () => void }) { const item = JOURNEY[step]; return <section className="dl44-room-body dl44-journey"><header><span>Experience Preview · synthetic adapter</span><h3>Homepage to a human next step.</h3><p>No live engine response or booking state is claimed.</p></header><ol>{JOURNEY.map((entry, index) => <li key={entry.place} aria-current={index === step ? "step" : undefined}><button onClick={() => onStep(index)}><span>{String(index + 1).padStart(2, "0")}</span>{entry.place}</button></li>)}</ol><article><span>{item.place}</span><h4>{item.title}</h4><p>{item.copy}</p><button onClick={() => step === 3 ? onThreeD() : onStep(Math.min(step + 1, JOURNEY.length - 1))}>{item.action}</button>{step > 0 ? <button onClick={() => onStep(step - 1)}>Back</button> : null}</article></section>; }

function ImplantExhibit({ stage, part, labels, onStage, onPart, onLabels }: { stage: number; part: "fixture" | "abutment" | "crown"; labels: boolean; onStage: (stage: number) => void; onPart: (part: "fixture" | "abutment" | "crown") => void; onLabels: (labels: boolean) => void }) { const selected = IMPLANT_3D_CONTRACT.components.find((item) => item.id === part)!; return <section className="dl44-room-body dl44-model-room"><header><span>{IMPLANT_3D_CONTRACT.assetId} · {IMPLANT_3D_CONTRACT.evidence}</span><h3>A museum-grade frame, ready for the real model.</h3><p>{IMPLANT_3D_CONTRACT.disclaimer}</p></header><div className="dl44-model-layout"><div className="dl44-model-stage" data-stage={stage}><div className="dl44-jaw"><i className="dl44-fixture" data-active={part === "fixture"} /><i className="dl44-abutment" data-active={part === "abutment"} /><i className="dl44-crown" data-active={part === "crown"} />{labels ? <><span className="label-fixture">Fixture</span><span className="label-abutment">Abutment</span><span className="label-crown">Crown</span></> : null}</div><strong>{IMPLANT_3D_CONTRACT.states[stage]}</strong><small>Code-native synthetic proxy · final GLB not present</small></div><aside><span>Selected component</span><h4>{selected.label}</h4><p>{selected.explanation}</p><div>{IMPLANT_3D_CONTRACT.components.map((item) => <button key={item.id} aria-pressed={part === item.id} onClick={() => onPart(item.id)}>{item.label}</button>)}</div><label>Teaching stage<input type="range" min="0" max="8" value={stage} onChange={(event) => onStage(Number(event.target.value))} /><small>{IMPLANT_3D_CONTRACT.states[stage]}</small></label><button onClick={() => onLabels(!labels)}>{labels ? "Hide labels" : "Show labels"}</button><details><summary>Read instead</summary><p>{IMPLANT_3D_CONTRACT.components.map((item) => `${item.label}: ${item.explanation}`).join(" ")}</p></details></aside></div><footer><span>Frozen action family</span><p>{CONCIERGE_3D_ACTIONS.join(" · ")}</p><small>{IMPLANT_3D_CONTRACT.replacementRule}</small></footer></section>; }
