...
export async function HeroRendererV2(props: HeroRendererV2Props) {
  ...
  const normalized = (rawFlag ?? "")
    ...
    .replace(/^'(.*)'$/, "$1")
    .toLowerCase();
  const resolvedProps = props.pageSlugOrPath
    ? props
    : (() => {
        const headersList = headers();
        return headersList.then((resolvedHeaders) => {
          const requestUrl = resolvedHeaders.get("next-url") ?? "";
          const pathname = requestUrl
            ...
            : "/";
          return { ...props, pageSlugOrPath: pathname };
        });
      })();

  const v2PropsWithPath = await resolvedProps;
  const v2Model = await buildHeroV2Model(v2PropsWithPath);

  if (!v2Model) return null;
