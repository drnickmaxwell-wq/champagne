...
export default function PatientAiPanel() {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-dashed border-neutral-200 bg-white p-6">
      <p className="text-sm font-medium text-neutral-800">Aftercare guidance request</p>
      ...
      <label className="space-y-1 block">
        <span className="text-sm text-neutral-800">Question or concern</span>
        <input
          type="text"
          value={symptom}
          onChange={(event) => setSymptom(event.target.value)}
          className="w-full rounded-lg border border-neutral-200 px-3 py-2 text-sm text-neutral-900"
          placeholder="e.g. swelling near an extraction site"
        />
      </label>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {loading ? "Preparing guidance..." : "Request guidance"}
        </button>
      </div>

      {error ? <p className="text-sm text-neutral-700">We could not prepare guidance just now: {error}</p> : null}

      {adviceLines.length > 0 ? (
        <div className="space-y-2 rounded-lg border border-neutral-200 bg-white p-3">
          <p className="text-sm font-medium text-neutral-800">Guidance</p>
