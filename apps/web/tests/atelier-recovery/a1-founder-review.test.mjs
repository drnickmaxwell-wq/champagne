...
test("completed Founder corpus reconciles all 331 archive items and preserves full history", () => {
  ...
  assert.equal(dataset.datasetRevision, 558);
  assert.equal(dataset.decisions.length, 595);
  assert.equal(currentDecisionMap(dataset).size, 331);
  assert.equal(progress.decided, 331);
...
test("Founder Decisions 51 through 57 remain verbatim in the completed current corpus", () => {
  ...
  const expected = [
    ...
    ["CVA-HERITAGE-B039-E01", "LOVE", "its my building"],
  ...
  assert.ok(initial.decisions.some((decision) => decision.cvaId === "CVA-HERITAGE-B039-E01" && decision.notes === "no its not . i know what picvture it is and will LOVE it" && decision.status === "SUPERSEDED"));
...
test("editing a completed item persists without implying trait evidence", () => {
  const target = archive[0];
  const next = applyReview(initial, target.id, { wholeItemSignal: "LOVE" }, "2026-08-13T19:00:00.000Z");
  const decision = currentDecisionMap(next).get(target.id);
...
test("undo creates a new supersession record and restores the previous state", () => {
  ...
  const before = currentDecisionMap(initial).get(cvaId);
  const changed = applyReview(initial, cvaId, { wholeItemSignal: "NOT_ME", notes: "temporary" }, "2026-08-14T01:03:00.000Z");
  const undone = undoLast(changed, "2026-08-14T01:04:00.000Z");
  const current = currentDecisionMap(undone).get(cvaId);
  assert.equal(current.wholeItemSignal, before.wholeItemSignal);
  assert.equal(current.notes, before.notes);
  assert.equal(current.source.kind, "SUPERSESSION");
  assert.equal(undone.decisions.filter((decision) => decision.cvaId === cvaId).length, initial.decisions.filter((decision) => decision.cvaId === cvaId).length + 2);
});
...
test("derived future-consumer index is deterministic and retains explicit evidence only", () => {
  ...
  assert.equal(first.filter((row) => row.wholeItemSignal !== "UNRATED").length, 331);
...
test("malformed, unknown, duplicate-current and stale datasets fail closed", () => {
  ...
  const duplicate = structuredClone(initial); duplicate.decisions.push({ ...duplicate.decisions.find((decision) => decision.status === "CURRENT"), decisionId: "duplicate" });
...
test("A2 exposes only experimental reconstruction while page generation and production binding stay unreachable", async () => {
  ...
  assert.match(source, /A2 EXPERIMENTAL RECONSTRUCTIONS · NO PAGE DESIGN/);
  assert.match(source, /ReconstructionLibrary/);
