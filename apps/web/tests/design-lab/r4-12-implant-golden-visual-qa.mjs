...
await frame().locator("body").evaluate(() => {
  window.addEventListener("message", event => {
    if (event.data?.type === "CHAMPAGNE_ATELIER_PREVIEW_STATE") window.__ATELIER_R412_QA_STATE__ = event.data.state;
  });
});
const captureFullPage = async path => {
  const state = await frame().locator("body").evaluate(() => window.__ATELIER_R412_QA_STATE__);
  assert.ok(state, "preview state must be observed before full-page capture");
